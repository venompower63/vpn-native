...
class VPNApp {
	...
	updateConnectionStatus(connected, connecting = false) {
		...
		if (this.elements.statusSubtext)
			this.elements.statusSubtext.textContent = connected
	...
	connect() {
		...
		setTimeout(() => {
			...
			if (this.elements.location)
				this.elements.location.textContent = "🇩🇪 Frankfurt";
